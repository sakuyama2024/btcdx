...
var (
	...
	// mainPowLimit is the highest proof of work value an Alphacash block can
	// have for the main network.  It is the value 2^228 - 1.
	mainPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 228), bigOne)

	// regressionPowLimit is the highest proof of work value an Alphacash block
	// can have for the regression test network.  It is the value 2^255 - 1.
	regressionPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 255), bigOne)

	// testNetPowLimit is the highest proof of work value an Alphcash block
	// can have for the test network.  It is the value 2^224 - 1.
	testNetPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 224), bigOne)

	// simNetPowLimit is the highest proof of work value an Alphacash  block
...
// MainNetParams defines the network parameters for the main alpha network.
var MainNetParams = Params{
	...
	SubsidyReductionInterval: 210000 * 5,
	TargetTimespan:           time.Hour * 24 * 14 / 5, // 14 days /5
...
var RegressionNetParams = Params{
	...
	TargetTimespan:           time.Hour * 24 * 14 / 5, // 2 weeks /5
	TargetTimePerBlock:       time.Minute * 2,         // 2 minutes
	RetargetAdjustmentFactor: 4,                       // 25% less, 400% more
	...
	// Address encoding magics
	PubKeyHashAddrID: 0x00, // starts with m or n ALPHA TODO
...
// SimNetParams defines the network parameters for the simulation test alpha
...
var (
	// ErrDuplicateNet describes an error where the parameters for an alpha
	// network could not be set due to the network already being a standard
	// network or previously-registered into this package.
	ErrDuplicateNet = errors.New("duplicate alpha network")
...
var (
	registeredNets       = make(map[wire.AlphaNet]struct{})
...
// Register registers the network parameters for an Alphacash network.  This may
